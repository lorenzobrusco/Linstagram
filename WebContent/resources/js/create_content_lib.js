...
//	console.log(container)
  ...
  $(document).ready(function () {
	...
	dropzoneOptions = {
			...
			// FIXME: sembra che sovrascrive l'evento precedente precedente
				this.on("addedfile", function(file) { 
						//FIXME ma qui ci vuole la tilde??
...
//					myDropzone.processQueue(); 
					if(~file[0].type.indexOf("video")){
						...
						if(type == "story"){
							console.log("ok");
							endFilter(canvas);
							UploadPicStory(canvas,filename); //NON FUNZIONA PIU PROBLEMI CON BLOB
						}
						else
							UploadFile(file[0],filename, "image",container);
					} else {
						UploadPic(canvas,filename); //NON FUNZIONA PIU PROBLEMI CON BLOB
					}
				});
